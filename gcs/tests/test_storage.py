...
def test_COO():
    ...
    s2 = COO((2, 3), Strided.fromobject([[0, 0, 0, 1, 1, 1], [0, 1, 2, 0, 1, 2]]), Buffer.fromobject([1, 2, 3, 4, 5, 6]))
    assert s == s2
    assert s.is_full()
    assert s[0, 0] == 1
    assert s[0, 1] == 2
    ...
    s2 = COO((2, 3), Strided.fromobject([[0, 0, 1], [0, 2, 1]]), Buffer.fromobject([1, 3, 5], unspecified=0))
    assert s == s2
    assert not s.is_full()
...
def test_FlatCOO():
    ...
    s2 = FlatCOO((2, 3), Buffer.fromobject([0, 1, 2, 3, 4, 5]), Buffer.fromobject([1, 2, 3, 4, 5, 6]))
    assert s == s2
    assert s.is_full()
    assert s[0, 0] == 1
    assert s[0, 1] == 2
...
def test_2D_GCS(storage_clsname, reduction):
    ...
    seq = [[1, 2, 3], [4, 5, 6]]
    s = GCS.fromobject(seq, dimensions=dimensions, partitioning=partitioning, storage_class=storage_class)
    ...
    seq = [[1, 0, 3], [0, 5, 0]]
    s = GCS.fromobject(seq, dimensions=dimensions, partitioning=partitioning, storage_class=storage_class, unspecified=0)
...
def generate_partitioning(N):
    for reduction_dimensionality in range(1, N+1):
        for partitioning in itertools.combinations(tuple(range(1, N)), r=reduction_dimensionality-1):
            yield partitioning
    ...
    for storage_class in storage_classes:
        ...
        for dimensions in sorted(selected_permutations):
            ...
            for partitioning in sorted(selected_partitioning):
                yield f'{storage_class}-{".".join(map(str,dimensions))}-{".".join(map(str,partitioning))}'
